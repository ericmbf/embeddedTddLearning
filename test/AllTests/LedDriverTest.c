...
TEST(LedDriver, test_allLedOffsOnCreate)
{
    ...
    TEST_ASSERT_EQUAL(invertByteOrder(0xFFFF), uc_virtualLeds);
}

TEST(LedDriver, test_TurnOnLedOne)
{
    LedDriver_turnOn(1);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0x01), ~gui_virtualLeds);
}
...
TEST(LedDriver, test_TurnOffLedOne)
{
    LedDriver_turnOn(1);
    LedDriver_turnOff(1);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0x00), ~gui_virtualLeds);
}
...
TEST(LedDriver, test_TurnOnMultipleLeds)
{
    LedDriver_turnOn(1);
    LedDriver_turnOn(2);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0x03), ~gui_virtualLeds);
}
...
TEST(LedDriver, test_TurnOffAnyLed)
{
    LedDriver_turnAllOn();
    LedDriver_turnOff(2);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0xFFFD), ~gui_virtualLeds);
}

TEST(LedDriver, test_allOn)
{
    LedDriver_turnAllOn();
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0xFFFF), ~gui_virtualLeds);
}
...
TEST(LedDriver, test_LedMemoryIsNotReadable)
{
    ...
    LedDriver_turnOn(8);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0x80), ~gui_virtualLeds);
}
...
TEST(LedDriver, test_UpperAndLowerBounds)
{
    LedDriver_turnOn(1);
    LedDriver_turnOn(16);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0x8001), ~gui_virtualLeds);
}

TEST(LedDriver, test_TurnOnOutOfBounds)
{
    LedDriver_turnOn(-1);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0), ~gui_virtualLeds);
    LedDriver_turnOn(0);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0), ~gui_virtualLeds);
    LedDriver_turnOn(17);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder(0), ~gui_virtualLeds);
...
TEST(LedDriver, test_TurnOffMultipleLeds)
{
    ...
    LedDriver_turnOff(9);
    LedDriver_turnOff(8);
    TEST_ASSERT_EQUAL_HEX16(invertByteOrder((~0x180) & 0xFFFF), ~gui_virtualLeds);
}
